...
def test_floatmatmult_scheduler_test():
    ...
    with grpc.insecure_channel('localhost:50051') as channel:
        ...
        floatmatmul_input = "matrix1_6000_0.5.txt"
...
def run_measurement_experiment():
    ...
    output_df = pd.DataFrame(columns=['function_type', 'function_input', 'predicted_cpu', 'predicted_frequency', 'predicted_energy', 'predicted_confidence', 'prediction_type', 'energy', 'duration', 'cpu_util', 'prediction_time'])
    ...
    with grpc.insecure_channel(f'localhost:{port}') as bayesian_channel:
        ...
        for _ in range(5000):
            ...
            predicted_energy = response.energy
            predicted_confidence = response.confidence
            prediction_type = response.prediction_type

            print(type(predicted_frequency))
            print(type(predicted_cpu))
            print(type(predicted_energy))
            print(type(predicted_confidence))
            print(type(prediction_type))

            const_memory = 4096

            print(f"Predicted freq: {predicted_frequency}, predicted cpu: {predicted_cpu}, predicted energy: {response.energy}, confidence: {response.confidence}, prediction type: {response.prediction_type}")
    ...
    output_df.to_csv("final_results_on_measurement_data_matern_0.1.csv")
...
def run_final_experiment(memory_baseline=None):
    ...
    output_df = pd.DataFrame(columns=['function_type', 'function_input', 'predicted_cpu', 'predicted_frequency', 'predicted_energy', 'predicted_confidence', 'prediction_type', 'prediction_time', 'memory'])
    ...
    file_path = f'../data/constructed-azure-traces/azure_trace_rps_{5}.csv'
    df_trace = pd.read_csv(file_path)

    memory_type = None
    if memory_baseline is None:                    
        memory_type = "CONSTANT"
    elif memory_baseline == SystemType.AQUATOPE:
        memory_type = "AQUATOPE"
    elif memory_baseline == SystemType.PARROTFISH:
        memory_type = "PARROTFISH"
    else:
        memory_type = "CONSTANT"
    ...
    with grpc.insecure_channel(f'localhost:{port}') as bayesian_channel:
        ...
        with grpc.insecure_channel('localhost:50051') as lachesis_channel:
            ...
            for index, row in df_trace.iterrows():
                print(f"Count: {count}")
                count += 1
                start_timestamp = row['start_timestamps'] * 5
                selected_function = row['function']
                slo = float(row['duration']) * (1 + SLO_MULTIPLIER)
                parameter_list = eval(row['parameters'])

                if selected_function not in ['floatmatmult', 'imageprocess', 'videoprocess', 'encrypt', 'linpack']:
                    continue

                print(f"Running {selected_function} with input: {parameter_list} and SLO: {slo}")
                ...
                response = None
                try:
                    response = bayesian_stub.predictCPUFrequency(bayesian_regressor_pb2.predictCPUFrequencyRequest(function=selected_function, inputs=parameter_list, slo=slo))
                except Exception as e:
                    # Log the traceback to your server's log system
                    print(traceback.format_exc())
                    # Optionally, send a detailed error message back to the client
                ...
                if memory_baseline is None:                    
                    ...
                    memory = AQUATOPE_MEM_REGISTRATIONS[selected_function]
                elif memory_baseline == SystemType.PARROTFISH:
                    memory = PARROTFISH_MEM_REGISTRATIONS[selected_function]
                else:
                    memory = 4096
                ...
                print(f"Predicted freq: {predicted_frequency}, predicted cpu: {predicted_cpu}, predicted energy: {response.energy}, confidence: {response.confidence}, prediction type: {response.prediction_type}, with memory: {memory}")

                response = lachesis_stub.Invoke(lachesis_pb2.InvokeRequest(function=selected_function, slo=slo, parameters=parameter_list, cpu=predicted_cpu, memory=memory, frequency=predicted_frequency))

                # print(f"Function: {selected_function}, Response: {response}")

                data_to_add = pd.DataFrame({
                    'function_type': [selected_function],
                    'function_input': [parameter_list],
                    'predicted_cpu': [predicted_cpu],
                    'predicted_frequency': [predicted_frequency],
                    'predicted_energy': [predicted_energy],
                    'predicted_confidence': [predicted_confidence],
                ...
                output_df = pd.concat([output_df, data_to_add], ignore_index=True)
                output_df.to_csv(f"final_results_{memory_type}.csv")
        
    output_df.to_csv(f"final_results_{memory_type}.csv")

    db_conn.close()
    print('Completed final experiment')


def delete_registered_baselines(system):
    AQUATOPE_MEM_REGISTRATIONS = {'linpack': 1849, 'floatmatmult': 3947, 'lrtrain': 2724, 'videoprocess': 834, 'imageprocess': 617, 'sentiment': 633, 'encrypt': 681, 'mobilenet': 562}
    ...
    for function in ['linpack', 'floatmatmult', 'videoprocess', 'imageprocess', 'encrypt']:
        if system == SystemType.AQUATOPE:
            memory = AQUATOPE_MEM_REGISTRATIONS[function]
            cpu = AQUATOPE_CPU_REGISTRATIONS[function]
        elif system == SystemType.PARROTFISH:
            memory = PARROTFISH_MEM_REGISTRATIONS[function]
            cpu = 1 + ((memory - 512) / (5760 - 512)) * (31 - 1)
            cpu = int(cpu)
        else:
            print("the chosen system is not supported")
            break
            

        fxn_deletion_cmd = 'wsk action delete {}_{}_{}_{}'.format(function, cpu, memory, 0)
        ...
        if 'ok: deleted action' not in fxn_del_out:
            print("Error deleting function: {}".format(function))
        else:
            print("Deleted function: {}_{}_{}_{}".format(function, cpu, memory, 0))

def register_baseline(system):
    ...
    with grpc.insecure_channel('localhost:50051') as channel:
        lachesis_stub = lachesis_pb2_grpc.LachesisStub(channel)

        parameters = ['endpoint:\"10.52.2.197:9002\"', 'access_key:\"testkey\"', 'secret_key:\"testsecret\"', 'bucket:\"openwhisk\"']

        functions = [FunctionType.FLOATMATMULT, FunctionType.IMAGEPROCESS, FunctionType.VIDEOPROCESS, FunctionType.LINPACK, FunctionType.ENCRYPT]

        function_to_function_map = {
            FunctionType.FLOATMATMULT: 'floatmatmult',
            FunctionType.IMAGEPROCESS: 'imageprocess',
            FunctionType.VIDEOPROCESS: 'videoprocess',
            FunctionType.LINPACK: 'linpack',
            FunctionType.ENCRYPT: 'encrypt'
        }
            
        for case in functions:

            selected_function = function_to_function_map[case]
            ...
            if system == SystemType.AQUATOPE:
                memory = AQUATOPE_MEM_REGISTRATIONS[selected_function]
                cpu = AQUATOPE_CPU_REGISTRATIONS[selected_function]
            elif system == SystemType.PARROTFISH:
                memory = PARROTFISH_MEM_REGISTRATIONS[selected_function]
                cpu = 1 + ((memory - 512) / (5760 - 512)) * (31 - 1)
                cpu = int(cpu)
            ...
            if (case == FunctionType.ALL) or (case == FunctionType.IMAGEPROCESS):
                # Image Process
                function_metadata = ['docker:psinha25/main-python']
                response = lachesis_stub.Register(lachesis_pb2.RegisterRequest(function='imageprocess', 
            ...
            if (case == FunctionType.ALL) or (case == FunctionType.FLOATMATMULT):
                # Floatmatmult
                function_metadata = ['docker:psinha25/main-python']
                response = lachesis_stub.Register(lachesis_pb2.RegisterRequest(function='floatmatmult',
                                                                    function_path='~/lachesis-2.0/benchmarks/functions/matmult',
                                                                    function_metadata=function_metadata,
                                                                    parameters=parameters,
                                                                    memory=memory,
            ...
            if (case == FunctionType.ALL) or (case == FunctionType.VIDEOPROCESS):
                ...
                response = lachesis_stub.Register(lachesis_pb2.RegisterRequest(function='videoprocess',
            ...
            if (case == FunctionType.ALL) or (case == FunctionType.LINPACK):
                # Linpack
                function_metadata = ['docker:psinha25/main-python']
                response = lachesis_stub.Register(lachesis_pb2.RegisterRequest(function='linpack',
            ...
            if (case == FunctionType.ALL) or (case == FunctionType.ENCRYPT):
                # Encryption
                function_metadata = ['docker:psinha25/main-python']
                response = lachesis_stub.Register(lachesis_pb2.RegisterRequest(function='encrypt',
        ...
        for case in functions:
            ...
            for cpu in CPUS:
                for frequency in FREQUENCIES:
                    for memory_type in ["AQUATOPE", "PARROTFISH", "CONSTANT"]:
                        if memory_type == "AQUATOPE":
                            memory = AQUATOPE_MEM_REGISTRATIONS[selected_function]
                        elif memory_type == "PARROTFISH":
                            memory = PARROTFISH_MEM_REGISTRATIONS[selected_function]
                        else:
                            memory = 4096
...
def run_baseline(system):
    ...
    with grpc.insecure_channel('localhost:50051') as lachesis_channel:
        ...
        for index, row in df_trace.iterrows():
            ...
            if selected_function not in inputs:
                continue
            ...
            print(f"Running {selected_function} with input: {parameter_list}, SLO: {slo}, cpu: {cpu}, memory: {memory}")

            response = None

            # invoke
            response = lachesis_stub.Invoke(lachesis_pb2.InvokeRequest(function=selected_function, slo=slo, parameters=parameter_list, cpu=cpu, memory=memory, frequency=0))
...
if __name__=='__main__':
    ...
    # register_baseline(SystemType.AQUATOPE)
    # run_baseline(SystemType.AQUATOPE)
    # register_final_experiment()
    run_final_experiment(SystemType.AQUATOPE)

    # delete_registered_baselines(SystemType.PARROTFISH)
